...
void loadMap(const char* dataPath) {
	...
	for (uint8_t i = 0; i < cJSON_GetArraySize(enemyData); i += 13) {
		newEnemy(
			...
			cJSON_GetArrayItem(enemyData, i + 6)->valueint,
			cJSON_GetArrayItem(enemyData, i + 7)->valueint,
			cJSON_GetArrayItem(enemyData, i + 8)->valueint,
			cJSON_GetArrayItem(enemyData, i + 9)->valueint,
			cJSON_GetArrayItem(enemyData, i + 10)->valuestring
		);
	}
